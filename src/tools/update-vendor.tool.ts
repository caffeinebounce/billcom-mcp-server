...
const toolDescription = "Update an existing vendor in Bill.com.";

const toolSchema = z.object({
  id: z.string().describe("The vendor ID to update (required)"),
  name: z.string().optional().describe("The vendor name"),
...
export const UpdateVendorTool: ToolDefinition<typeof toolSchema> = {
  ...
  handler: async (args) => {
    ...
    const response = await updateVendor(params);
